...
defineProps({tweet: Object, user: Object});

const deleteTweet = (tweetId) => {
    router.delete(`/tweets/${tweetId}/delete`)
}
...
<template>
    <div class="min-w-[60px]">
        <img class="rounded-full m-2 mt-3" width="50" :src="/storage/ + user.image">
    </div>
    <div class="p-2 w-full">
        <div class="font-extrabold flex items-center justify-between mt-0.5 mb-1.5">
            <div class="flex items-center" >
                <div>{{ user.id == tweet.user_id ? user.name : tweet.name }}</div>
            ...
            <div class="hover:bg-gray-800 rounded-full cursor-pointer relative"
                ...
                <div v-if="openOptions"
                    ...
                    <ul class="p-3">
                        <button @click="deleteTweet(tweet.id)" class="flex items-center cursor-pointer">
                            <TrashCanOutline class="pr-3" fillColor="#DC2626"
                                             :size="18"/>
